...
json_file1 <- "talks_art.json"
json_file2 <- "talks_free.json"
json_file3 <- "talks_music.json"
json_file4 <- "talks_time.json"

###Read a json file to workspace 
a <-fromJSON(file=json_file1, method="C") 
b <-fromJSON(file=json_file2, method="C") 
c <-fromJSON(file=json_file3, method="C") 
d <-fromJSON(file=json_file4, method="C") 

talk_creativ <- c(a,b,c,d)
...
talk_names 
###Convertobject type 
talk_creativ_list<-lapply(c(a$talks,b$talks,c$talks,d$talks), function(x){unlist(x)}) 
...
ted_docs <- tm_map(ted_docs, removeWords, "music")
###Tokenizing 
...
class(stem_docs) 
###term weight: TfIDF 
ted_tdm <- TermDocumentMatrix(stem_docs, control = list(removePunctuation = TRUE, weighting=weightTfIdf, stopwords = TRUE)) 
inspect(ted_tdm[1,]) 
...
png("./dendrogram_sparse94_ward.png", width = 1200, height=600) 
plot(fit) 
###k= number of clusters 
rect.hclust(fit, k=7) 
dev.off() 
...
write.table(df_groups, "./tc_result_ward.txt", row.names=FALSE, col.names=TRUE, sep="\t") 
